...
router.get('/', (req, res) => {
    try {
        ...
        if (limit) {
            filteredStats = filteredStats.slice(0, parseInt(limit));
        }
        res.json({
            success: true,
            data: filteredStats,
...
router.post('/match/:matchId', (req, res) => {
    try {
        const { matchId } = req.params;
        const { player_id, batting_stats, bowling_stats, fielding_stats } = req.body;
        ...
        const existingStats = matchPlayerStats.find(stat => stat.match_id === matchId && stat.player_id === player_id);
        if (existingStats) {
...
router.put('/match/:matchId/player/:playerId', (req, res) => {
    try {
        ...
        const { batting_stats, bowling_stats, fielding_stats } = req.body;
        const matchStatsIndex = matchPlayerStats.findIndex(stat => stat.match_id === matchId && stat.player_id === playerId);
...
router.get('/performance', (req, res) => {
    try {
        ...
        playerStatsMap.forEach((stat, playerId) => {
            if (team_id && stat.team_id !== team_id)
                return;
        ...
        // Apply limit if specified
        if (limit) {
            performances = performances.slice(0, parseInt(limit));
        }
        res.json({
            success: true,
            data: performances,
    ...
    try {
        ...
        teamStatsMap.forEach((teamStat, teamId) => {
            ...
            if (teamPlayerStats.length > 0) {
                ...
                // Find top performers
                const topBatsman = teamPlayerStats.reduce((prev, current) => prev.runs_scored > current.runs_scored ? prev : current);
                const topBowler = teamPlayerStats.reduce((prev, current) => prev.wickets_taken > current.wickets_taken ? prev : current);
...
function calculateRecentForm(stat) {
    // Simple form calculation based on recent performance
    if (stat.batting_average > 50 && stat.bowling_average < 25)
        return 'excellent';
    if (stat.batting_average > 30 && stat.bowling_average < 35)
        return 'good';
    if (stat.batting_average > 20 && stat.bowling_average < 45)
        return 'average';
    return 'poor';
}
function calculateAchievements(stat) {
    const achievements = [];
    if (stat.centuries > 0)
        achievements.push(`${stat.centuries} century${stat.centuries > 1 ? 's' : ''}`);
    if (stat.five_wickets > 0)
        achievements.push(`${stat.five_wickets} five-wicket haul${stat.five_wickets > 1 ? 's' : ''}`);
    if (stat.half_centuries > 0)
        achievements.push(`${stat.half_centuries} half-century${stat.half_centuries > 1 ? 's' : ''}`);
    if (stat.strike_rate > 150)
        achievements.push('High strike rate');
    if (stat.economy_rate < 4)
        achievements.push('Economical bowler');
    return achievements;
}
