...
router.get('/', (req, res) => {
    try {
        const { team_id, match_type, status, date_from, date_to, venue, result_type, limit = '20', offset = '0' } = req.query;
        let filteredMatches = [...matchHistory];
        // Apply filters
        if (team_id) {
            filteredMatches = filteredMatches.filter(match => match.team1_id === team_id || match.team2_id === team_id);
        }
        if (match_type) {
        ...
        if (venue) {
            filteredMatches = filteredMatches.filter(match => match.venue?.toLowerCase().includes(venue.toLowerCase()));
        }
        if (result_type) {
            filteredMatches = filteredMatches.filter(match => match.result.result_type === result_type);
        }
...
router.get('/head-to-head/:team1Id/:team2Id', (req, res) => {
    try {
        ...
        const headToHeadMatches = matchHistory.filter(match => (match.team1_id === team1Id && match.team2_id === team2Id) ||
            (match.team1_id === team2Id && match.team2_id === team1Id));
        const team1Wins = headToHeadMatches.filter(match => match.result.winner === team1Id).length;
        const team2Wins = headToHeadMatches.filter(match => match.result.winner === team2Id).length;
        const ties = headToHeadMatches.filter(match => match.result.result_type === 'tied').length;
        res.json({
            success: true,
            data: {
                team1_id: team1Id,
...
router.get('/venue/:venue/stats', (req, res) => {
    try {
        ...
        const venueMatches = matchHistory.filter(match => match.venue?.toLowerCase().includes(venue.toLowerCase()));
        const venueStats = {
            venue,
...
function generateMatchHistorySummary(matches) {
    const completedMatches = matches.filter(m => m.status === 'completed');
    const liveMatches = matches.filter(m => m.status === 'live');
    const scheduledMatches = matches.filter(m => m.status === 'scheduled');
    const totalRuns = completedMatches.reduce((sum, match) => sum + match.result.innings_summary.reduce((teamSum, innings) => teamSum + innings.total_runs, 0), 0);
    const totalWickets = completedMatches.reduce((sum, match) => sum + match.result.innings_summary.reduce((teamSum, innings) => teamSum + innings.total_wickets, 0), 0);
    const averageScore = completedMatches.length > 0 ? totalRuns / completedMatches.length : 0;
    const highestScore = Math.max(...completedMatches.map(match => Math.max(...match.result.innings_summary.map(innings => innings.total_runs))));
    return {
...
function calculateAverageScore(matches) {
    if (matches.length === 0)
        return 0;
    const totalRuns = matches.reduce((sum, match) => sum + match.result.innings_summary.reduce((teamSum, innings) => teamSum + innings.total_runs, 0), 0);
    return totalRuns / matches.length;
}
function calculateHighestScore(matches) {
    if (matches.length === 0)
        return 0;
    return Math.max(...matches.map(match => Math.max(...match.result.innings_summary.map(innings => innings.total_runs))));
}
function calculateMostCommonResult(matches) {
    if (matches.length === 0)
        return 'No matches';
    ...
    return Object.entries(resultCounts).reduce((a, b) => resultCounts[a[0]] > resultCounts[b[0]] ? a : b)[0];
}
