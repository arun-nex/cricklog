...
import { Team, Player, CreateTeamRequest, CreatePlayerRequest } from '../models/Team';
...
router.post('/', (req, res) => {
  try {
    const { name, description, captain }: CreateTeamRequest = req.body;
...
router.post('/:id/players', (req, res) => {
  try {
    ...
    const { name, role, batting_style, bowling_style, jersey_number }: CreatePlayerRequest = req.body;
